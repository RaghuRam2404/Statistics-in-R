...
```{r}
ggplot(data=mlb11, aes(x=homeruns,y=wins)) + geom_point()
```
...
```{r homeruns-vs-runs}
homeruns_vs_runs <- lm(runs ~ homeruns, data = mlb11)
homeruns_vs_runs
```
```{r}
summary(homeruns_vs_runs)
```
...
```{r residual}
at_bats_vs_runs <- lm(runs~at_bats, data=mlb11)
summary(at_bats_vs_runs)
```
...
```{r runs-vs-sth-else}
ggplot(data = mlb11, aes(x = homeruns, y = runs)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
