...
```{r gained-mature-ht}
inference(x=mature, y=gained, data=nc, statistic="mean", type="ht", null=0, method="theoretical", alternative = "twosided")
```

7. Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.
```{r}
summary(nc %>% filter(mature =="mature mom") %>% select(mage))
```

```{r}
inference(y = mage, data = nc %>% filter(mature =="mature mom"), statistic = "mean", type = "ci", method = "theoretical")
```

```{r}
summary(nc %>% filter(mature =="younger mom") %>% select(mage))
```

```{r}
inference(y = mage, data = nc %>% filter(mature =="younger mom"), statistic = "mean", type = "ci", method = "theoretical")
```
