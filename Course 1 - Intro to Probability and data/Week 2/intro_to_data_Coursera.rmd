...
sfo_feb_flights %>% summarize(median = median(dep_delay))
```
...
nycflights %>% group_by(month) %>% summarize(mean = mean(dep_delay)) %>% arrange(desc(mean))
```
...
nycflights %>% group_by(month) %>% summarize(median= median(dep_delay)) %>% arrange(desc(median))
```
...
nycflights <- nycflights %>% mutate(qten = ifelse(dep_type=="delayed", ifelse(arr_type=="on time", 1, 0), 0))
nycflights %>% summarise(proporation = sum(qten)/n())
```