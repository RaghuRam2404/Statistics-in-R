...
```
```{r}
ggplot(data=nc, aes(y=weight,x="1")) + geom_boxplot()
```
...
```{r}
nc %>% group_by(premie) %>% summarise(count = n())
```
```{r habit-weight-box}
ggplot(nc %>% filter(premie=="full term"), aes(x = habit, y = weight)) +
  geom_boxplot()
```
...
bayes_inference(y = weight, x = habit, data = nc_fullterm, 
                ...
                type = "ht", alternative = "twosided", null = 0, 
                prior = "JZS", rscale = 1, 
                method = "theoretical", show_plot = FALSE, hypothesis_prior=c(.25, .75))
```
...
```{r Q11-ht-sport-diff}
bayes_inference(y = diff, data = PrisonStress %>%
  filter(Group == "Sport") %>%
  mutate(diff = PSSbefore - PSSafter), 
                statistic = "mean",
                type = "ht", alternative = "twosided", null = 0, 
