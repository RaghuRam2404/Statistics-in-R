...
```{r Q3}
ames_train %>% filter(Neighborhood == 'StoneBr' | Neighborhood == 'Timber' | Neighborhood == 'Veenker' | Neighborhood == 'NridgHt') %>% group_by(Neighborhood) %>% summarise(sd1 = sd(price))
...
```{r Q4}
ggplot(data=ames_train, aes(x=Lot.Area, y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
ggplot(data=ames_train, aes(x=Bedroom.AbvGr, y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
ggplot(data=ames_train, aes(x=Overall.Qual, y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
ggplot(data=ames_train, aes(x=Year.Built, y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
...
```{r Q5}
ggplot(data=ames_train, aes(x=area, y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
summary(lm(price ~ area, data=ames_train))$adj.r.squared
ggplot(data=ames_train, aes(x=area, y=log(price))) + geom_point() + stat_smooth(method = "lm", se = FALSE)
summary(lm(log(price) ~ area, data=ames_train))$adj.r.squared
ggplot(data=ames_train, aes(x=log(area), y=price)) + geom_point() + stat_smooth(method = "lm", se = FALSE)
summary(lm(price ~ log(area), data=ames_train))$adj.r.squared
ggplot(data=ames_train, aes(x=log(area), y=log(price))) + geom_point() + stat_smooth(method = "lm", se = FALSE)
summary(lm(log(price) ~ log(area), data=ames_train))$adj.r.squared
...
```{r Q6}
atleast_one_garage <- ames_train %>% filter(!is.na(Garage.Type)) %>% summarise(n=n())
atleast_one_garage+9
1000-atleast_one_garage+1
```
...
```{r Q7}
ames_train %>% filter(Year.Built > 1999) %>% summarise(percent = n()/1000)
ames_train %>% summarise(mean = mean(price), median = median(price), is_med_greater_than_mean = median > mean)
ames_train %>% filter(is.na(Bsmt.Qual)) %>% summarise(count = n(), twentyone = count==21)
...
ames_train[o[1:3],] %>% select(price, Sale.Condition, Bedroom.AbvGr, Overall.Qual, Year.Built)
```
...
```{r Q12}
ames_train %>% group_by(Neighborhood) %>% summarise(mean=mean(Bldg.Type == "1Fam")) %>% arrange(desc(mean))
...
```{r Q13}
ggplot(data=ames_train, aes(x=Bedroom.AbvGr, y=log(price))) + geom_point(pch=1, color="red", alpha=0.5) + stat_smooth(method = "lm", se = FALSE)
ames_train %>% summarise(cor=cor(log(price), Bedroom.AbvGr))
```
