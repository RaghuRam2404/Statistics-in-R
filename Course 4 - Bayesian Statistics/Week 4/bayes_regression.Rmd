...
outliers_new <- Bayes.outlier(m_lwage_iq, k = 4.034)
outliers_new_df <- data.frame(probability = outliers_new$prob.outlier,
                          case = 1:length(outliers_new$prob.outlier))
ggplot(outliers_new_df, aes(ymax = probability, x = case)) +
  geom_linerange(ymin = 0) +
  labs(y = "Probability")

outliers_new_df %>% filter(probability > 0.50)
...
```{r Q7-BIC}
m_lwage_nobrthord_ques <- lm(lwage ~ . - wage - brthord, data = na.omit(wage))
BIC(m_lwage_nobrthord_ques)
m_lwage_nobrthord_ques <- lm(lwage ~ . - wage - sibs, data = na.omit(wage))
BIC(m_lwage_nobrthord_ques)
m_lwage_nobrthord_ques <- lm(lwage ~ . - wage - feduc, data = na.omit(wage))
BIC(m_lwage_nobrthord_ques)
m_lwage_nobrthord_ques <- lm(lwage ~ . - wage - meduc, data = na.omit(wage))
BIC(m_lwage_nobrthord_ques)
...
```{r Q12-cred-interval}
HPM_pred_lwage <- predict(bma_lwage, estimator = "HPM", se.fit = TRUE)
ci_hpm_lwage <- confint(HPM_pred_lwage, estimator = "HPM")
opt_hpm <- which.max(ci_hpm_lwage)
str(opt_hpm)
